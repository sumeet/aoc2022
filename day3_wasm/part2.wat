...
(module
  ...
  (func $solve (result i64)
    (local $str_i i32)
    (local $group_counter i32)
    (local $sum i64)
    (local $group_bitfield i64)
    (local $this_bitfield i64)

    (block $solve_loop
      (loop $line_loop
        ;; bust out of the loop if we're at EOF 
        (i32.eqz (call $getc (local.get $str_i)))
        (br_if $solve_loop)

        (call $priority (call $getc (local.get $str_i)))
        (call $2_to_the_n)

        (i64.or (local.get $this_bitfield))
        (local.set $this_bitfield)

        (local.set $str_i
          (i32.add (local.get $str_i) (i32.const 1)))
        (if 
          ;; hit a newline
          (i32.eq (call $getc (local.get $str_i))
                  (i32.const 10))
          (then
            ;; if this is the first group, then
            (if (i32.eqz (local.get $group_counter))
              (then
                ;; set the group bitfield, to this bitfield
                (local.get $this_bitfield)
                (local.set $group_bitfield)
              )
              (else
                ;; otherwise AND into group_bitfield
                (i64.and
                  (local.get $group_bitfield)
                  (local.get $this_bitfield))
                (local.set $group_bitfield)
              )
            )
            ...
            (if
              (i32.eq (local.get $group_counter)
                      (i32.const 2))
              (then
                ;;(local.get $group_bitfield)
                ;;(call $print_int_backwards)
                ...
                (i64.ctz (local.get $group_bitfield))
                (i64.add (local.get $sum))
                (local.set $sum)

                (local.set $group_counter (i32.const 0))
              )
              (else
                (local.set $group_counter
                  (i32.add (local.get $group_counter)
                           (i32.const 1)))
              )
            )

            ;; reset $this_bitfield
            (local.set $this_bitfield (i64.const 0))
            ;; jump past the newline
            (local.set $str_i
              (i32.add (local.get $str_i) (i32.const 1)))
            (br $line_loop)
          )
          ;; not equal to 10, keep adding to this bitfield
          (else
            (br $line_loop)
          )
        )
  ...
  ;; same as $print_int_backwards but puts pipes on the side
  (func $dbg_int_backwards (param $num i64)
    (call $putc (i32.const 124))
    (loop $loop
      ...
      (i64.gt_u (local.get $num) (i64.const 0))
      br_if $loop
    )
