...
(module
  ...
  (func $score_win_lose_draw (param $abc i32) (param $xyz i32)
        (result i64)
    (local $abc_rps i32)
    (local $xyz_rps i32)
    ;; $abc - 65
    ...
    ;; check if it's a draw ; return 3
    (if (i32.eq (local.get $abc_rps) (local.get $xyz_rps))
      (then
        (i64.const 3)
        return))
    ...
    ;; 0 means they won
    ;; they have rock, we have scissors
    (if (i32.and
    ...
    (if (i32.and
      ...
      (then
        (i64.const 0)
        return))
    ;; they have paper, we have scissors
    (if (i32.and
          (i32.eq (local.get $abc_rps) (i32.const 2))
          (i32.eq (local.get $xyz_rps) (i32.const 1)))
      (then
        (i64.const 0)
        return))
  ...
  (func $score_xyz (param $char i32) (result i64)
    (i64.sub
      (i64.extend_i32_s (local.get $char))
      (i64.const 87))
  )
